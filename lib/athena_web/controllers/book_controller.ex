defmodule AthenaWeb.BookController do
  ...
  def graph(conn, _params) do
    query = from b in Book,
      ...
      order_by: fragment("to_char(?, 'YY/MM')", b.read_at),
      group_by: fragment("to_char(?, 'YY/MM')", b.read_at),
      select: %{month: fragment("to_char(?, 'YY/MM')", b.read_at), books: count(b.id), pages: fragment("?::integer", avg(b.pages))}
